...
describe("buildRefreshScalars — no data loss, atomic-write payload", () => {
  it("sets apply_value + guide + variance when the API HAS a value for the grade", () => {
    const scalars = buildRefreshScalars({ final_value_cents: 4250 }, value({ guide_value_cents: 5000 }), "exact");
  ...
  it("apply_value=false when the API has no value — the DB then PRESERVES a hand-entered guide", () => {
    ...
    const scalars = buildRefreshScalars({ final_value_cents: 4250 }, value({ guide_value_cents: null }), "exact");
  ...
  it("carries NO Final / Quick-Sale / Replacement / confidence key (approved valuation is structurally untouchable)", () => {
    const scalars = buildRefreshScalars({ final_value_cents: 4250 }, value(), "exact");
    ...
    expect("valuation_confidence" in scalars).toBe(false);
  });

  it("variance is null when there is no final value even if the guide is present", () => {
    const scalars = buildRefreshScalars({ final_value_cents: null }, value({ guide_value_cents: 5000 }), "exact");
    expect(scalars.apply_value).toBe(true);
    expect(scalars.value_cents).toBe(5000);
    expect(scalars.variance).toBeNull();
  });
