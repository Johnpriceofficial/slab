 ...
 * failed panel shows an inline "X failed to load" message with an in-place
 * "Try again" (no full page reload -- it just re-renders the subtree, which
 * recovers a transient failure, e.g. a momentary bad API response, without
 * losing the rest of the page's state). Deliberately simpler than the
 * top-level ErrorBoundary -- no chunk-load auto-reload logic, since a panel
 * already inside a loaded page is not itself a lazy route chunk that could
 * 404 after a deploy.
 */
...
export class PanelErrorBoundary extends Component<PanelErrorBoundaryProps, PanelErrorBoundaryState> {
  ...
  // Re-renders the failing subtree in place. If the underlying cause was
  // transient (a flaky fetch, a momentary bad response) this recovers
  // without a full page reload; if it's deterministic it simply falls back
