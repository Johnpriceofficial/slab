...
export default function Signup() {
  ...
  const [error, setError] = useState<string | null>(null); const [sent, setSent] = useState(false); const [submitting, setSubmitting] = useState(false);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (captcha.enabled && !captcha.token) return setError("Complete the security verification before creating your account.");
    setSubmitting(true); setError(null);
    const result = await signUp(email.trim(), password, captcha.token || undefined);
    setSubmitting(false);
    if (result.error) { setError(result.error); captcha.reset(); return; }
    setSent(result.needsVerification);
  };
  return <main className="container flex min-h-screen max-w-md flex-col justify-center py-12"><PageHead title="Create account · GradedCardValue.com" noindex /><Card><CardHeader><CardTitle>Create your account</CardTitle></CardHeader><CardContent>{sent ? <div className="space-y-4 text-sm"><p className="rounded-md border border-green-600/30 bg-green-600/5 p-3 text-green-700">Check your email and open the verification link before signing in.</p><Button className="w-full" asChild><Link to="/login">Return to sign in</Link></Button></div> : <form className="space-y-4" onSubmit={submit}>
    ...
    <Field label="Confirm password" id="signup-confirm" type="password" autoComplete="new-password" value={confirm} setValue={setConfirm} />
    ...
    <Button type="submit" className="w-full" disabled={submitting || (captcha.enabled && !captcha.token)}>{submitting && <Loader2 className="animate-spin" />} Create account</Button>
